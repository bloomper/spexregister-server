...
class TagApiTest extends AbstractApiTest {
    ...
    void should_get_events() throws Exception {
        final var event1 = EventDto.builder().id(1L).eventType(Event.EventType.CREATE.name()).sourceType(Event.SourceType.TAG.name()).build();
        final var event2 = EventDto.builder().id(2L).eventType(Event.EventType.UPDATE.name()).sourceType(Event.SourceType.TAG.name()).build();
        ...
        mockMvc
                .perform(
                        get("/api/tags/events/{sourceId}?sinceInDays=30", 1)
                                .apiVersion("1.0")
                                .header(HttpHeaders.AUTHORIZATION, "Bearer token")
                                .header(HttpHeaders.ACCEPT_LANGUAGE, "en")
                )
                ...
                .andDo(
                        document(
                                ...
                                secureRequestHeaders,
                                responseHeaders,
                                security(getRolesFromMethod(TagApi.class, "retrieveEvents", Long.class, Integer.class))
                        )
                );
    }
