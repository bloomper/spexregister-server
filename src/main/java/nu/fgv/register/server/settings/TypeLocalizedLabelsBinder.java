 ...
 * @since 2.0
 */
public class TypeLocalizedLabelsBinder implements TypeBinder {
    ...
    public void bind(final TypeBindingContext context) {
        ...
        try (final BeanHolder<SpexregisterConfig> config = context.beanResolver().resolve(SpexregisterConfig.class, BeanRetrieval.ANY)) {
            ...
            for (final String lang : config.get().getLanguages()) {
                fieldReferences.put(lang, root.field("labels_" + lang, f -> f.asString().aggregable(Aggregable.YES)).toReference());
            }

            context.bridge(Type.class, new Bridge(fieldReferences));
        }
    }

    private record Bridge(Map<String, IndexFieldReference<String>> fieldReferences) implements TypeBridge<Type> {

        @Override
        public void write(final DocumentElement target, final Type bridgedElement, final TypeBridgeWriteContext context) {
            final String id = bridgedElement.getId();

            bridgedElement.getLabels().forEach((lang, label) -> {
