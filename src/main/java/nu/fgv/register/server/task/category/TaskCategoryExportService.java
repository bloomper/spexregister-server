...
public class TaskCategoryExportService extends AbstractExportService {
    ...
    public TaskCategoryExportService(final TaskCategoryService service, final List<ExportEngine> engines) {
        super(engines);
    ...
    protected List<ReportHolder<?>> getReports(final List<Long> ids, final String filter) {
        return List.of(
                ReportHolder.of(
                        toImpexDto(service.streamByIds(ids, filter, Sort.by(Sort.Direction.ASC, "name")), TASK_CATEGORY_MAPPER::toImpexDto),
