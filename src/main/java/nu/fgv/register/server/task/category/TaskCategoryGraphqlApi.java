...
public class TaskCategoryGraphqlApi {
    ...
    public Window<TaskCategoryDto> retrieve(final ScrollSubrange subrange, @Argument final Optional<String> filter, final Optional<Sort> sort) {
    ...
    public JobReferenceDto export(@Argument final List<Long> ids, @Argument final String filter, @Argument final ExportType type, final Locale locale) {
        return JobReferenceDto.builder()
                .id(jobService.createExportJob(TaskCategoryExportService.class, ids, filter, type, locale))
                .build();
    }
